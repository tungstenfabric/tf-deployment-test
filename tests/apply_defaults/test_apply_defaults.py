...
class ApplyDefaultsTests(WithAttributes, DeploymentTestCase, JujuAction):
    ...
    def test_juju_action_apply_defaults(self):
        ...
        self.logger.info(f'original_juju.split(): {original_juju_encap_priorities.split(",")}')
        try:
            assert original_juju_encap_priorities.split(",") == original_api_encap_priorities, "ERROR: juju encap_priority differs from api encap_priority"
            ...
            assert new_juju_encap_priorities.split(",") == current_api_encap_priorities, "ERROR: api encap_priority was not set or set incorrectly"
        finally:
