...
class ApplyDefaultsTests(WithAttributes, DeploymentTestCase, JujuAction):
    ...
    def test_juju_action_apply_defaults(self):
        ...
        self.logger.info(f'original_juju.strip().split(): {original_juju_encap_priorities.strip().split(",")}')
        assert original_juju_encap_priorities.strip().split(",") == original_api_encap_priorities, "ERROR 1_One"
        ...
        assert new_juju_encap_priorities.strip().split(",") == current_api_encap_priorities, "ERROR 4_Three"
