...
echo "" >> ziu_log
echo "run started" >> ziu_log
$my_dir/../../../rhosp/$RHOSP_DIR/run.sh

run_exit_code=$?
echo "rhosp/$RHOSP_DIR/run.sh exited $run_exit_code" >> ziu_log
if [[ $run_exit_code != 0 ]]; then
    echo "rhosp/$RHOSP_DIR/run.sh exited $run_exit_code"
    exit 1
fi
echo "wait started" >> ziu_log
