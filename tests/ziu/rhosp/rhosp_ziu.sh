...
function check_new_tag() {
    ...
    for machine in $(echo "$AGENT_NODES,$CONTROLLER_NODES" | sed "s/,/ /g"); do
        if [[ $(ssh $SSH_OPTIONS $machine "sudo contrail-status | sed '/^$/q' | sed '1d;\$d' | awk '{print \$4}' " | grep -v "^abc$CONTRAIL_NEW_IMAGE_TAG$") != "" ]]; then
            echo "ERROR:  $machine's contrail-status contains something other than '$CONTRAIL_NEW_IMAGE_TAG' tag" | tee -a ziu_test.log
            return 1
        else
            echo "INFO:   $machine successfully changed its tag in contrail-status" | tee -a ziu_test.log
        fi
    done
}
...
echo "" | tee -a ziu_test.log
echo "$(date) run started" | tee -a ziu_test.log

$my_dir/../../../rhosp/$RHOSP_DIR/run.sh

run_exit_code=$?
echo "$(date) rhosp/$RHOSP_DIR/run.sh exited $run_exit_code" | tee -a ziu_test.log

if [[ $run_exit_code != 0 ]]; then
    echo "$(date) ziu process failed. have a look at ~/ziu_run.log"
    exit 1
fi

echo "$(date) wait started" | tee -a ziu_test.log
...
echo "$(date) wait finished" | tee -a  ziu_test.log
echo "$(date) check contrail-status on overcloud nodes" | tee -a ziu_test.log

check_new_tag

echo "$(date) all overcloud nodes feel good. ZIU test finished" | tee -a ziu_test.log
