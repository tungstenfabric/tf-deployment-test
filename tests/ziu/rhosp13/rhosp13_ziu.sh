...
function check_tf_active() {
  local machine
  local line=
  #local regex="(overcloud-contrailcontroller|overcloud-controller|overcloud-novacompute)"
  #local server_list=$(openstack server list --name overcloud-contrailcontroller -c Networks -f value && openstack server list --name overcloud-novacompute -c Networks -f value )

  #for machine in $(openstack server list -c Networks -f value | cut -d "=" -f2  | sort -u) ; do
...
./$HOME/tf-deployment-test/rhosp/rhosp13_ziu/run.sh
