...
echo "$(date)  $overcloud_ctrlcont_prov_ip | $overcloud_cont_prov_ip | $overcloud_compute_prov_ip" >> z_ziu_log
tls_env_files=''
...
echo "$(date) upgrading nodes" >> z_ziu_log

echo "$(date)  $overcloud_ctrlcont_instance | $overcloud_cont_instance | $overcloud_compute_instance" >> z_ziu_log
for node in $(echo "$overcloud_ctrlcont_instance,$overcloud_cont_instance,$overcloud_compute_instance" | sed "s/,/ /g"); do
