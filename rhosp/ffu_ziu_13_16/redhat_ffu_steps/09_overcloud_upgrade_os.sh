...
openstack overcloud external-upgrade run $force --stack overcloud --tags ceph_systemd \
  -e ceph_ansible_limit=$pcs_bootstrap_node
openstack overcloud upgrade run $force --stack overcloud --tags system_upgrade --limit $pcs_bootstrap_node
openstack overcloud external-upgrade run $force --stack overcloud --tags system_upgrade_transfer_data
openstack overcloud upgrade run $force --stack overcloud --playbook upgrade_steps_playbook.yaml --tags nova_hybrid_state --limit all
retry 3 openstack overcloud upgrade run $force --stack overcloud --limit $pcs_bootstrap_node
...
for node in $(openstack server list --name overcloud-controller -c Name -f value | grep -v "$pcs_bootstrap_node" ) ; do
  ...
  openstack overcloud upgrade run $force --stack overcloud --limit $upgraded_controllers
done
